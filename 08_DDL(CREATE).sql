...
-- 문자 인코딩 UTF-8: 영어/숫자 = 1BYTE, 한글/특수문자 = 3BYTE
...
-- 추가 샘플 데이터 삽입
-- 가입일 -> SYSDATE를 활용
INSERT INTO "MEMBER" VALUES('MEM02', 'QWER1234', '김영희', '970506-2234567', SYSDATE);

-- 가입일 -> DEFAULT 활용(테이블 생성 시 정의된 값이 반영됨)
INSERT INTO "MEMBER" VALUES('MEM03', 'ASDF9876', '곽철수', '940416-1987654', DEFAULT);

-- 가입일 -> INSERT 시 미작성 하는 경우 -> DEAFULT 값이 반영됨
INSERT INTO "MEMBER"(MEMBER_ID, MEMBER_PWD, MEMBER_NAME) 
VALUES('MEM04', '000AAA', '이지연');
...
--> NUMBER타입 컬럼에 데이터 삽입 시 제일 앞에 0이 오면 이를 자동으로 제거함
--  --> 전화번호, 주민등록번호 처럼 숫자로만 되어 있는 데이터지만
...
CREATE TABLE USER_USED_UK(
    USER_NO NUMBER,
    /*컬럼 레벨*/
--  USER_ID VARCHAR2(20) UNIQUE, -- 제약조건명 미지정
...
-- NOT NULL + UNIQUE 제약조건의 의미 -> 중복되지 않는 값이 필수로 존재 해야함
...
--> 기본키 중복으로 오류
-- ORA-00001: 무결성 제약 조건(KH_SBK.USER_NO_PK)에 위배됩니다

INSERT INTO USER_USED_PK
VALUES(NULL, 'user03', 'pass03', '유관순', '여', '010-9999-3131', 'yoo123@kh.or.kr');
--> 기본키가 NULL 이므로 오류
-- ORA-01400: NULL을 ("KH_SBK"."USER_USED_PK"."USER_NO") 안에 삽입할 수 없습니다
...
-- ORA-00001: 무결성 제약 조건(KH_SBK.PK_USERNO_USERID)에 위배됩니다
...
-- ORA-02291: 무결성 제약조건(KH_SBK.GRADE_CODE_FK)이 위배되었습니다- 부모 키가 없습니다
...
WHERE GRADE_CODE = 30;
-- ORA-02292: 무결성 제약조건(KH_SBK.GRADE_CODE_FK)이 위배되었습니다- 자식 레코드가 발견되었습니다
...
-- **CHECK 제약 조건은 FK 제약 조건의 하위 호환이다
--> 둘 다 컬럼의 값을 지정된 형태로 제한 하지만 
--  CHECK는 작성 후 수정이 번거롭지만
--  FK는 부모 테이블에 행을 추가, 수정, 삭제 하는 게 간편함
...
INSERT INTO USER_TEST VALUES(1,'USER01','PW01','930518-2111111','여','01033334444','서울시 도봉구','N');
INSERT INTO USER_TEST VALUES(2,'USER02','PW02','970905-1034445','남','01012341234','서울시 강남구','N');
INSERT INTO USER_TEST VALUES(3,'USER03','PW03','900326-1111333','남','01098767890','서울시 중구','N');
INSERT INTO USER_TEST VALUES(4,'USER04','PW04','991208-2090909','여','01045678765','안양시 동안구','N');
INSERT INTO USER_TEST VALUES(5,'USER05','PW05','870124-1234567','남','01023456789','서울시 중랑구','N');
...
-- 1) 테이블 전체 복사
--CREATE TABLE EMPLOYEE_COPY AS 서브쿼리
...
--  제약조건, 코멘트는 복사되지 않기 때문에 별도 추가작업 필요!
