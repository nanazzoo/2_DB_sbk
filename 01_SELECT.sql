...
-- EMPLOYEE 테이블에서 모든 사원의 사번, 이름, 급여 * 12(연봉) 조회
...
SELECT EMP_NAME, HIRE_DATE, SYSDATE FROM EMPLOYEE e ;
...
-- --> ""(쌍따옴표)는 특수문자, 대소문자, 기호 등을 구분하여 나타낼 때 사용하는 표기법
...
-- EMPLOYEE 테이블에서 부서 코드가 'D9'인 사원의
-- 사번, 이름, 부서코드, 직급코드 조회
...
-- NOT BETWEEN A AND B: A미만 또는 B초과
SELECT EMP_ID, EMP_NAME, SALARY
FROM EMPLOYEE e 
...
-- EMPLOYEE 테이블에서 성이 '전'씨인 사원의 사번, 이름을 조회
...
/* NULL 처리 연산자
 ...
 JAVA에서의 NULL: 참조하는 객체가 없음을 의미하는 값
 DB에서의   NULL: 컬럼에 값이 없음을 의미하는 값
...
/* ORDER BY 절
	...
	** SELECT문 해석 시 가장 마지막에 해석된다 **
	...
	1. FROM절
	2. WHERE절
	4. ORDER BY 컬럼명 | 별칭 | 컬럼 순서(숫자) [ASC | DESC] [NULLS FIRST | LAST]
											   (오름 / 내림 차순)
 */
...
SELECT EMP_ID, EMP_NAME, SALARY
FROM EMPLOYEE
ORDER BY SALARY; -- 기본값: ASC(오름차순)


-- 급여 200만 이상인 사원의 사번, 이름, 급여 조회 단, 급여 내림차순
SELECT EMP_ID, EMP_NAME, SALARY
FROM EMPLOYEE
WHERE SALARY >= 2000000
ORDER BY 3 DESC ; -- 3 = SELECT절에서 3번째 언급 된 컬럼명
