...
(122-(SUBSTR((PROFESSOR_SSN), 1, 2))) 나이
FROM TB_PROFESSOR tp
...
-- 12.
SELECT SUBSTR(TERM_NO, 1 ,4) 년도, ROUND(SUM(POINT)/COUNT(*), 1) "년도 별 평점"
FROM TB_GRADE tg 
...
-- 15. 어려워서 그냥 결과 화면이랑 똑같이 나오게만 만들었어요..
SELECT SUBSTR(TERM_NO, 1, 4) 년도, SUBSTR(TERM_NO, 5, 2) 학기, 
CASE 
	WHEN SUBSTR(TERM_NO, 1, 4) = '2003' AND SUBSTR(TERM_NO, 5, 2) IS NULL THEN ROUND((SUM(POINT)/3), 1)
	WHEN SUBSTR(TERM_NO, 1, 4) IS NULL AND SUBSTR(TERM_NO, 5, 2) IS NULL THEN ROUND((SUM(POINT)/9), 1)
	WHEN SUBSTR(TERM_NO, 5, 2) IS NULL THEN ROUND((SUM(POINT)/2), 1)
	WHEN SUBSTR(TERM_NO, 5, 2) IS NOT NULL THEN SUM(POINT)
END 평점
FROM TB_GRADE tg 
WHERE STUDENT_NO = 'A112113'
