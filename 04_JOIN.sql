...
- 관계형 데이터베이스는 최소한의 데이터를 테이블에 담고 있어
  ...
  두 테이블에서 '같은 데이터'를 저장하는 컬럼이 연결고리가됨.   
*/
...
-- 사번, 이름, 사수의 사번, 사수 이름
-- ANSI 표준
SELECT E.EMP_ID, E.EMP_NAME, NVL(E.MANAGER_ID, '없음'), NVL(M.EMP_NAME, '-')
FROM EMPLOYEE E
...
--				  EMPLOYEE와 DEPARTMENT가 조인된 결과를 먼저 만들고
...
-- ANSI
SELECT EMP_ID, EMP_NAME, DEPT_TITLE, JOB_NAME, LOCAL_NAME, SALARY
FROM EMPLOYEE
JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID)
JOIN JOB USING(JOB_CODE)
JOIN LOCATION ON(LOCATION_ID = LOCAL_CODE)
...
WHERE e.DEPT_CODE = e2.DEPT_CODE AND e.EMP_NAME != e2.EMP_NAME
ORDER BY 사원명;
