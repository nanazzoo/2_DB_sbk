...
ALTER TABLE DEPT_COPY ADD CONSTRAINT DEPT_UK UNIQUE(DEPT_TITLE);

-- DEPT_COPY의 DEPT_TITLE 컬럼에 UNIQUE 삭제
ALTER TABLE DEPT_COPY DROP CONSTRAINT DEPT_UK;

-- ***DEPT_COPY의 DEPT_TITLE 컬럼에 NOT NULL 제약조건 추가/삭제***
ALTER TABLE DEPT_COPY ADD CONSTRAINT DEPT_TITLE_NN NOT NULL(DEPT_TITLE); -- 불가
...
--   테이블은 최소 한개 이상의 컬럼이 존재해야 하기 때문에 
--	 모든 컬럼을 삭제할 수는 없다.
...
--  3) DROP TABLE 삭제 옵션인 CASCADE CONSTRAINTS 사용
...
--> DDL은 수행 시 존재하는 트랜잭션을 모두 DB에 강제 COMMIT 시킴
--> DDL이 종료된 후 DML 구문을 수행하도록 권장
