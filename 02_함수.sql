...
SELECT ROUND(123.456, 0) FROM DUAL; -- 123, 0 = 기본 값 (소수점 첫 번째 자리에서 반올림)

SELECT ROUND(123.456, -1) FROM DUAL; -- 120, 일의 자리에서 반올림

SELECT ROUND(123.456, -2) FROM DUAL; -- 100, 십의 자리에서 반올림

SELECT ROUND(123.456, -3) FROM DUAL; -- 0, 백의 자리에서 반올림
...
SELECT CEIL(123.1), FLOOR(123.9) FROM DUAL; -- CEIL = 124, FLOOR = 123 


-- TRUNC(숫자 | 컬럼명 [,위치]) : 특정 위치 아래를 버림(절삭)
SELECT TRUNC(123.456) FROM DUAL; -- 123, 소수점 아래 버림 

SELECT TRUNC(123.456, 1) FROM DUAL; -- 123.4, 소수점 첫 번째 자리 아래 버림 

SELECT TRUNC(123.456, -1) FROM DUAL; -- 120, 십의 자리 아래 버림 
...
SELECT ROUND(MONTHS_BETWEEN(SYSDATE ,'2022-08-04'), 3) "수강 기간(개월)" FROM DUAL;
-- 0.918개월

-- EMPLOYEE 테이블에서 사원의 이름, 입사일, 근무한 개월 수, 근무 년 차 조회
SELECT EMP_NAME, HIRE_DATE,
CEIL(MONTHS_BETWEEN(SYSDATE, HIRE_DATE)) "근무한 개월 수",
CEIL(MONTHS_BETWEEN(SYSDATE, HIRE_DATE)/12) || '년 차' "근무 년 차"
FROM EMPLOYEE;
...
SELECT ADD_MONTHS(SYSDATE, -1) "1개월 전" FROM DUAL;  -- 음수도 가능

SELECT SYSDATE + 120 "120일 후" FROM DUAL;


-- LAST_DAY(날짜) : 해당 달의 마지막 날짜를 구함.
SELECT LAST_DAY(SYSDATE) "마지막 날" FROM DUAL; 

SELECT LAST_DAY('2020-02-01') "윤년" FROM DUAL;  
...
SELECT SYSDATE FROM DUAL; -- 2022-09-01 11:20:45.000

-- 2022/09/01 11:20:45 목요일
...
SELECT TO_DATE('2022-09-03')  FROM DUAL; -- DATE 타입으로 변환
SELECT TO_DATE(20220903)  FROM DUAL;
...
SELECT TO_DATE('510505', 'YYMMDD') FROM DUAL; -- 2049년
SELECT TO_DATE('510505', 'RRMMDD') FROM DUAL; -- 2049년
-- 기본값: RR
...
-- EMPLOYEE 테이블에서 보너스를 받으면 'O', 안 받으면 'X'를 조회
...
-- 직원의 성별 구하기(주민등록번호 뒤 첫째 자리 남:1 / 여:2)
...
-- 급여 최소값, 가장 오래된 입사일, 알파벳 순서가 가장 빠른 이메일 주소
SELECT MIN(SALARY), MIN(HIRE_DATE), MIN(EMAIL)
FROM EMPLOYEE e ;

-- 급여 최대값, 가장 최근 입사일, 알파벳 순서가 가장 느린 이메일 주소
SELECT MAX(SALARY), MAX(HIRE_DATE), MAX(EMAIL)
FROM EMPLOYEE e ;


-- EMPLOYEE 테이블에서 급여를 가장 많이받는 사원의 이름, 급여, 직급코드 조회
SELECT EMP_NAME 사원명, SALARY 급여, JOB_CODE 직급
FROM EMPLOYEE e 
...
SELECT COUNT(DISTINCT DEPT_CODE) FROM EMPLOYEE; -- 6명 


-- EMPLOYEE 테이블에서 성별이 남성인 사원의 수 조회 -- 15
SELECT COUNT(*) 
FROM EMPLOYEE 
