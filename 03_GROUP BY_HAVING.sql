...
SELECT DECODE(SUBSTR(EMP_NO, 8, 1), '1', '남', '2', '여') 성별, 
	...
	SUM(SALARY) "급여 합"
FROM EMPLOYEE
GROUP BY DECODE(SUBSTR(EMP_NO, 8, 1), '1', '남', '2', '여') -- 별칭 사용X (SELECT절 해석 아직X)
...
-- EMPLOYEE 테이블에서 직급 별 2000년도 이후(2000년도 포함) 입사자들의 급여 합을 조회
...
FROM EMPLOYEE
GROUP BY DEPT_CODE, JOB_CODE -- DEPT_CODE로 그룹을 나누고, 나눠진 그룹 내에서 JOB_CODE로 또 그룹을 분류
ORDER BY 1, 2 DESC;
...
FROM EMPLOYEE e 
-- WHERE SALARY >= 3000000 --> 한 사람의 급여가 300백만 이상이라는 조건(요구사항 불일치)
GROUP BY DEPT_CODE
HAVING ROUND(AVG(SALARY)) >= 3000000 --> DEPT_CODE 그룹 중 평균 급여가 300만원 이상인 그룹만 조회
ORDER BY 1;


-- EMPLOYEE 테이블에서 직급별 인원수가 5명 이하인 직급코드, 인원수 조회(직급 코드 오름차순)
